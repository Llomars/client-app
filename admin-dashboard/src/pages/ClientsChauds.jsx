...
function ClientsChauds() {
  ...
  const signedClientsByMonth = React.useMemo(() => {
    ...
    const signed = allClients.filter(
      (c) =>
        c.statut === 'Vendu' &&
        (Number(c.acompte1) > 0 || Number(c.acompte2) > 0)
    );
  ...
  return (
    <div style={{ padding: 40 }}>
      ...
      <div style={{ display: 'flex', gap: 18, marginBottom: 24 }}>
        <button
          style={{
            background: activeTab === 'chauds' ? '#6366f1' : '#e0e7ff',
            color: activeTab === 'chauds' ? '#fff' : '#3730a3',
            border: 'none',
            borderRadius: 8,
            padding: '10px 24px',
            fontWeight: 800,
        ...
        <button
          style={{
            background: activeTab === 'signes' ? '#6366f1' : '#e0e7ff',
            color: activeTab === 'signes' ? '#fff' : '#3730a3',
            border: 'none',
            borderRadius: 8,
            padding: '10px 24px',
            fontWeight: 800,
                ...
                <tbody>
                  {clients.map((client) => (
                    <tr
                      ...
                      style={{
                        background:
                          client.statut === 'Vendu' ? '#d1fae5' : '#f1f5f9',
                      }}
                    >
                      <td style={{ padding: 8 }}>
                        {client.nom} {client.prenom}
                      ...
                      <td style={{ padding: 8 }}>
                        {(() => {
                          ...
                          if (client.modeleCentrale) {
                            const kitObj = kits.find(
                              (k) => k.value === client.modeleCentrale
                            );
                            return kitObj
                              ? kitObj.label
                              : client.modeleCentrale;
                          }
                          return '-';
                        })()}
                      </td>
              ...
              >
                ...
                {clients.length > 0
                  ? (
                      ...
                      clients.reduce(
                        (acc, c) =>
                          acc + (Number(accomptes[c.id]?.acompte1) || 0),
                        0
                      )
                    ).toLocaleString() + ' €'
                  : '-'}
      ...
      {activeTab === 'signes' && (
        <div>
          ...
          >
            Clients signés et ayant versé un acompte (par mois)
          </h3>
          {/* Sélecteur de mois */}
          {Object.keys(signedClientsByMonth).length > 0 && (
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontWeight: 700, marginRight: 12 }}>
              ...
              >
                ...
                {Object.keys(signedClientsByMonth)
                  .sort()
                  .map((mois) => (
                    ...
                    <tbody>
                      {clients.map((client) => (
                        <tr key={client.id} style={{ background: '#f1f5f9' }}>
                          ...
                          <td style={{ padding: 8 }}>
                            {client.emailCommercial ||
                              client.emailManager ||
                              '-'}
                          </td>
                          <td style={{ padding: 8 }}>
                            {client.dateVente
                              ? new Date(client.dateVente).toLocaleDateString()
