...
export default function Relances() {
      ...
      if (userRole === 'commercial') {
        ...
        setRelances(
          snap.docs
            ...
            .filter((r) => clientIds.includes(r.clientId))
        );
        return;
      }
      const snap = await getDocs(q);
  ...
  const handleSaveRelance = async () => {
    ...
    try {
      ...
      setRelanceErreur(
        "Erreur lors de l'ajout de la relance : " + (err.message || err)
      );
      setSelectedClient(null);
      setDateRelance('');
      setCommentaire('');
  ...
  const relancesDuJour = relances.filter(
    (r) => r.dateRelance === todayStr && !r.effectuee
  );
  // Relances à venir (date future)
  const relancesAVenir = relances.filter(
    (r) => r.dateRelance > todayStr && !r.effectuee
  );
  // Relances en attente = toutes les relances passées ou du jour non effectuées
  const relancesEnAttente = relances.filter(
    (r) => r.dateRelance <= todayStr && !r.effectuee
  );
  ...
  const allRelancesFiltered = filterMonth
    ? allRelancesAVenir.filter(
        (r) => r.dateRelance && r.dateRelance.startsWith(filterMonth)
      )
    : allRelancesAVenir;

  // Liste des mois disponibles dans les relances à venir
  const moisDispo = Array.from(
    new Set(allRelancesAVenir.map((r) => r.dateRelance?.slice(0, 7)))
  ).sort();
    ...
    <div style={{ padding: 40 }}>
      ...
      {(userRole === 'admin' || userRole === 'manager') && (
        <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
          <button
            onClick={() => setRelanceTab('mes')}
            style={{
              padding: '8px 18px',
              background: relanceTab === 'mes' ? '#2563eb' : '#e5e7eb',
              color: relanceTab === 'mes' ? '#fff' : '#334155',
              border: 'none',
              borderRadius: 6,
              fontWeight: 600,
              fontSize: 16,
          ...
          <button
            onClick={() => setRelanceTab('toutesAVenir')}
            style={{
              padding: '8px 18px',
              background: relanceTab === 'toutesAVenir' ? '#2563eb' : '#e5e7eb',
              color: relanceTab === 'toutesAVenir' ? '#fff' : '#334155',
              border: 'none',
              borderRadius: 6,
              fontWeight: 600,
              fontSize: 16,
      ...
      (userRole === 'admin' || userRole === 'manager') ? (
        <>
          ...
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {allRelancesFiltered.length === 0 && (
              <li style={{ color: '#64748b', fontWeight: 600 }}>
        ...
        <>
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={{ fontWeight: 600 }}>Date de relance :</label>
            <br />
            <input
              type="date"
              value={dateRelance}
          ...
          <button
            ...
            disabled={!selectedClient || !dateRelance}
            style={{
              padding: '8px 18px',
              background: '#2563eb',
              color: '#fff',
          ...
          <div style={{ marginBottom: 18, display: 'flex', gap: 12 }}>
            <button
              onClick={() => setRelanceFilter('jour')}
              style={{
                padding: '8px 18px',
                background: relanceFilter === 'jour' ? '#2563eb' : '#e5e7eb',
                color: relanceFilter === 'jour' ? '#fff' : '#334155',
                border: 'none',
                borderRadius: 6,
                fontWeight: 600,
                fontSize: 16,
            ...
            <button
              onClick={() => setRelanceFilter('avenir')}
              style={{
                padding: '8px 18px',
                background: relanceFilter === 'avenir' ? '#2563eb' : '#e5e7eb',
                color: relanceFilter === 'avenir' ? '#fff' : '#334155',
                border: 'none',
                borderRadius: 6,
                fontWeight: 600,
                fontSize: 16,
            ...
            <button
              onClick={() => setRelanceFilter('enAttente')}
              style={{
                padding: '8px 18px',
                background:
                  relanceFilter === 'enAttente' ? '#ef4444' : '#e5e7eb',
                color: relanceFilter === 'enAttente' ? '#fff' : '#334155',
                border: 'none',
                borderRadius: 6,
                fontWeight: 600,
                fontSize: 16,
                      ...
                      >
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={!!r.effectuee}
                            onChange={() => handleToggleEffectuee(r)}
                          />{' '}
                          Effectuée
                        </label>
                      </div>
