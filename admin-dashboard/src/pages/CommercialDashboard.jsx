...
export default function CommercialDashboard() {
  ...
  const ventesMois = myClients.filter((c) => {
    if (!c.statut || (c.statut !== 'Vendu' && c.statut !== 'Signé'))
      return false;
        ...
        <div>
          ...
          >
            <summary style={{ cursor: 'pointer', fontWeight: 600 }}>
              Voir les clients pris en compte dans le dashboard
            </summary>
            <div style={{ fontSize: 14 }}>
              {myClients.length === 0 ? (
                'Aucun client trouvé pour ce commercial.'
              ) : (
                <ul>
                  {myClients.map((c) => (
                    <li key={c.id}>
                      ...
                      | Statut: {c.statut} | Prix: {c.prixCentrale} | Date
                      vente: {c.dateVente || '-'}
                      <br />
                      <span style={{ color: '#64748b' }}>
                        emailCommercial: <b>{c.emailCommercial || '-'}</b> |
                        emailCommercialAcco:{' '}
                        <b>{c.emailCommercialAcco || '-'}</b> | emailCom:{' '}
                        <b>{c.emailCom || '-'}</b>
                      </span>
                    </li>
                  ))}
          ...
          <div style={{ display: 'flex', gap: 20, marginBottom: 40 }}>
            ...
            >
              ...
              <div style={{ color: '#f59e42', fontSize: 28, fontWeight: 700 }}>
                {totalCommission
            ...
            >
              ...
              <div style={{ color: '#6366f1', fontSize: 28, fontWeight: 700 }}>
                {salaireTotal.toLocaleString('fr-FR', {
                  style: 'currency',
                  currency: 'EUR',
                })}
              </div>
...
function PerformanceSection({ clients, commerciaux }) {
  ...
  commerciaux.forEach((com) => {
    ...
    if (
      (role === 'commercial' || role === 'manager' || role === 'admin') &&
      com.email &&
      !seenEmails.has(com.email)
    ) {
  ...
  const topCommerciauxMois = statsMois
    .filter((com) => com.caMois > 0)
  ...
  const topCommerciauxAnnuel = statsAnnuel
    .filter((com) => com.caAnnee > 0)
    .sort((a, b) => b.caAnnee - a.caAnnee);
      ...
      <div>
        ...
        >
          <div style={{ color: '#6b7280', fontSize: 15 }}>
            CA total du mois (tous utilisateurs)
          </div>
          <div style={{ color: '#2563eb', fontSize: 30, fontWeight: 700 }}>
            {caTotalMois
              ? caTotalMois.toLocaleString('fr-FR', {
                  style: 'currency',
                  currency: 'EUR',
                })
              : '0 €'}
        ...
        >
          <span style={{ fontWeight: 600, fontSize: 16, color: '#334155' }}>
            Mois :
          </span>
          <select
            value={selectedMonth}
        ...
        >
          <summary style={{ cursor: 'pointer', fontWeight: 600 }}>
            Debug CA attribué par commercial
          </summary>
          <div style={{ fontSize: 14 }}>
            {statsMois.map((com) => (
              <div key={com.email} style={{ marginBottom: 16 }}>
                <b>
                  {com.displayName} ({com.email})
                </b>
                <ul>
                  {com.debugClients.length === 0 ? (
                    <li>Aucune vente attribuée</li>
                  ) : (
                    com.debugClients.map((c, idx) => (
                      <li key={idx}>
                        <span style={{ color: '#2563eb' }}>
                          Prix: <b>{c.prixCentrale}</b> | CA attribué:{' '}
                          <b>{c.caAttribue}</b>
                        </span>
                        <br />
                        <span style={{ color: '#64748b' }}>
                          ...
                          emailCommercialAcco:{' '}
                          <b>{c.emailCommercialAcco || '-'}</b> | emailCom:{' '}
                          <b>{c.emailCom || '-'}</b> | commercial courant:{' '}
                          <b>{c.emailCommercialCourant}</b>
                        </span>
                      </li>
                    ))
                  )}
        ...
        >
          {podiumMois.length === 0 ? (
            ...
            >
              Aucun vendeur n'a réalisé de CA ce mois-ci.
              <br />
              Motivez votre équipe pour apparaître sur le podium !
            </div>
          ) : (
            podiumMois.map((com, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 15, color: '#64748b', marginBottom: 6 }}
                  >
                    CA du mois
                  </div>
                  <div
                    style={{ fontSize: 22, color: '#2563eb', fontWeight: 700 }}
                  >
                    {com.caMois.toLocaleString('fr-FR', {
                      style: 'currency',
                      currency: 'EUR',
                    })}
                  </div>
        ...
        >
          ...
          {top10Mois.length === 0 ? (
            ...
            >
              Aucun vendeur n'a réalisé de CA ce mois-ci.
              <br />
              Le graphique s'affichera dès qu'une vente sera enregistrée.
            </div>
          ) : (
        ...
        >
          <h3 style={{ marginBottom: 16, color: '#334155' }}>
            Podium cumul annuel ({selectedYear})
          </h3>
          <div
            style={{
              display: 'flex',
          ...
          >
            {podiumAnnuel.length === 0 ? (
              ...
              >
                Aucun vendeur n'a réalisé de CA cette année.
                <br />
                Le podium annuel s'affichera dès qu'une vente sera enregistrée.
              </div>
            ) : (
              podiumAnnuel.map((com, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {com.caAnnee.toLocaleString('fr-FR', {
                        style: 'currency',
                        currency: 'EUR',
                      })}
                    </div>
          ...
          <h3 style={{ marginBottom: 16 }}>
            Top 10 vendeurs cumul annuel (CA)
          </h3>
...
function EquipeStatsSection() {
  ...
  React.useEffect(() => {
    getDocs(collection(db, 'clients')).then((snap) => {
      ...
      const managerEmails = [
        ...new Set(allClients.map((c) => c.emailManager).filter(Boolean)),
      ];
  ...
  const getEquipeStats = (managerEmail) => {
    const moisActuel = `${selectedYear}-${String(selectedMonth).padStart(
      2,
      '0'
    )}`;
    // Stats manager
    const statsManager =
      statsVendeurs.find(
        (s) => s.email === managerEmail && s.mois === moisActuel
      ) || {};
    // Stats commerciaux
    const teamCommerciaux = commerciaux.filter(
      (com) => com.managerEmail === managerEmail
    );
    const statsCommerciaux = teamCommerciaux.map((com) => {
      const statsCom =
        statsVendeurs.find(
          (s) => s.email === com.email && s.mois === moisActuel
        ) || {};
      return {
        ...com,
    ...
    const equipeClients = clients.filter(
      (c) => c.emailManager === managerEmail
    );
    ...
    return {
      ca,
      rdvPris: rdvPrisMois,
      rdvFaits: rdvFaitsMois,
      ventes,
      rdvFaitsMois,
      rdvPrisMois,
      commerciaux: statsCommerciaux,
      equipe: equipeClients,
    };
  };
  ...
  const renderDaySection = () => (
    <div style={{ marginBottom: 32 }}>
      <h3 style={{ color: '#334155', marginBottom: 12 }}>
          ...
          return (
            ...
            >
              ...
              >
                ...
                >
                  RDV pris : <b>{managerStats.nbRdvPris || 0}</b>
                </div>
                <div
                  style={{ fontSize: 14, color: '#64748b', marginBottom: 2 }}
                >
                  RDV faits : <b>{rdvFaitsJour}</b>
                </div>
                <div
                  style={{ fontSize: 14, color: '#64748b', marginBottom: 2 }}
                >
                  Vendus : <b>{ventesJour}</b>
                </div>
              </div>
              {stats.commerciaux.length === 0 ? (
                <div style={{ fontSize: 14, color: '#64748b' }}>
                  Aucun commercial attribué
                </div>
              ) : (
                stats.commerciaux.map((com) => {
                  ...
                  const rdvFaitsComJour = stats.equipe.filter((c) => {
                    ...
                    return (
                      rdvDate &&
                      c.emailCommercial === com.email &&
                      new Date(rdvDate).toISOString().slice(0, 10) ===
                        selectedDay
                    );
                  }).length;
                  const ventesComJour = stats.equipe.filter(
                    (c) =>
                      c.statut === 'Vendu' &&
                      c.emailCommercial === com.email &&
                      c.dateVente &&
                      new Date(c.dateVente).toISOString().slice(0, 10) ===
                        selectedDay
                  ).length;
                  return (
                    <div
        ...
        {managers.map((email) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ color: '#64748b', fontSize: 15, marginBottom: 8 }}>
                RDV pris : <b>{stats.rdvPris}</b>
              </div>
              <div style={{ color: '#64748b', fontSize: 15, marginBottom: 8 }}>
                RDV faits : <b>{stats.rdvFaits}</b>
              </div>
              <div style={{ color: '#64748b', fontSize: 15, marginBottom: 8 }}>
                Ventes : <b>{stats.ventes}</b>
              </div>
                ...
                {stats.commerciaux.length === 0 ? (
                  ...
                  stats.commerciaux.map((com) => (
                    ...
                    >
                      ...
                      >
                        Ventes du mois : <b>{com.ventesMois || 0}</b>
                      </div>
                      {/* Top professions : si dispo, sinon rien */}
                      {Array.isArray(com.topProfs) &&
        ...
        >
          ...
          >
            ...
            >
              ...
              <tbody>
                {getEquipeStats(selectedManager).equipe &&
                  getEquipeStats(selectedManager).equipe.map((c) => (
                    <tr key={c.id}>
                      <td>{c.nom}</td>
                      <td>{c.prenom}</td>
                      <td>{c.statut}</td>
...
function getRdvFaitDate(client) {
  ...
  if (client.etatProjet && client.etatProjet.rdvFaitDate)
    return client.etatProjet.rdvFaitDate;
